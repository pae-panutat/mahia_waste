            ...
            <script type="text/javascript" src="js/jscharts.js"></script>
                <br>
                    ...
                    <tr>
                        ...
                        <td><center>{{ number_format($airkwh,2) }}</center></td>
                        <td><center>{{ number_format($airpc*100,2) }}</center></td>
                    </tr>
                    <tr>
                        <td><center>แสงสว่าง</center></td>
                        <td><center>{{ number_format($elampkwh,2) }}</center></td>
                        <td><center>{{ number_format($elamppc*100,2) }}</center></td>
                    </tr>
                    <tr>
                        <td><center>อุปกรณ์ไฟฟ้า</center></td>
                        <td><center>{{ number_format($equipkwh,2) }}</center></td>
                        <td><center>{{ number_format($equippc*100,2) }}</center></td>                     
                    </tr>
                    <tr>
                        <td><center>อื่นๆ</center></td>
                        <td><center>{{ number_format($otherkwh,2) }}</center></td>
                        <td><center>{{ number_format($otherpc*100,2) }}</center></td>                      
                    </tr>
                    <tr>
                        <td><center>รวม</center></td>
                        <td><center>{{ number_format($totalkwh,2) }}</center></td>
                        <td><center>{{ number_format($totalpc*100,2) }}</center></td>                      
                    </tr>
                    </table>

                    <br>

                        <form action="{{url('/table4&'.$off_id.'&'.$year)}}">
                          ...
                          <select name="building_name" id="building_name">
                            ...
                            @foreach (DB::SELECT(DB::raw("SELECT * 
                                    ...
                                    WHERE off_id = $off_id AND year = $year ORDER BY building_name ASC ")); as $data)  
                            <option value="{{ $data->building_name }} "> <?php echo " ".$data->building_name; ?></option>
                            @endforeach
                            @endforeach 
                          </select>
                          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
